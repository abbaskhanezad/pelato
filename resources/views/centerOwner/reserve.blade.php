...
<style>
    ...
    .se-pre-con {
        ...
        height: 100%;
        z-index: 9999;
        background: url(https://mir-s3-cdn-cf.behance.net/project_modules/disp/35771931234507.564a1d2403b3a.gif) center no-repeat #fff;
    }
...
<script>
    function sbt() {
        ...
        var act = document.getElementsByClassName('active')[1].id;
        document.cookie = "getid = " + act;
                        ...
                        <div class="portlet-title">
                            <div class="row" style="padding-top: 70px;padding-bottom: 10px;">
                                <div class="caption font-dark" style="width: 450px;">
                                    ...
                                    <span class="caption-subject bold uppercase text-center "
                                          style="margin-right:15%; font-weight:bold;font-size:18px;font-family:'IRANSans-web';">زمانبندی اتاقهای مرکز " {{ $center->name }}
                                        "
                    </span>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="row col-md-3 col-md-offset-9">
                                        <a class="btn btn-warning" href="/centerowner/selectday"><span
                                                    class="glyphicon glyphicon-hand-right" aria-hidden="true"></span>بازگشت</a>
       ...
       Session::forget('active');
            ...
            <ul class="nav nav-tabs">
                ...
                @foreach($center->room as $room)
                    ...
                    <li id={{$i}} @if($i== $active_id) class="active" @endif><a data-toggle="tab" href="#br<?=$i ?>">اتاق: {{ $room->name }}
                            -{{$room->size}}متری</a></li>
            ...
            <div class="tab-content">
                ...
                @foreach($center->room as $room)
                    <div id="<?= 'br' . $k ?>" class="tab-pane fade @if($k== $active_id) {{ 'in active'}} @endif">
                        <div class="panel panel-info">
                            <div class="panel-heading"
                                 style="font-size: 18px; font-weight: bold; text-align: center;"> {{ $room->name }} </div>
                                            ...
                                            <div class="portlet-title">
                                                ...
                                                <div class="actions" style="display: none;">
                                                    ...
                                                    <a class="btn btn-default btn-sm" title="قیمت متفاوت اتاق">
                                                        ...
                                                        <input type="number"
                                                               style="width:60px;height:20px;display: inline"
                                                               name="room_{{ $room->id}}_new_price" min="1"
                                                               id="new_price" class="form-control"
                           ...
                           -->
                                                    ...
                                                    <table class="table table-striped table-bordered table-hover table-checkable order-column"
                                                        ...
                                                        <tr>
                                                            <th class="text-center" style="width: 30%">ساعت</th>
                                                        ...
                                                        @for($i=0;$i<24;$i++)
                                                            <tr>
                                                                ...
                                                                @foreach($day_mapper as $dmkey => $dm)
                                                                    @if($dmkey==$day_id-1)
                                                                        <th class="box room_{{ $room->id }} text-center"
                                                                            data-roomid="{{$room->id}}"
                                                                            data-day="{{$dmkey }}" data-hour="{{ $i }}"
                                                                            id="box_{{ $dmkey }}_{{ $i }}"></th>
                                                                    @else
                                                                        <th style="display:none;"
                                                                            class="box room_{{ $room->id }} text-center"
                                                                            data-roomid="{{$room->id}}"
                                                                            data-day="{{$dmkey }}" data-hour="{{ $i }}"
                                                                            id="box_{{ $dmkey }}_{{ $i }}"></th>
