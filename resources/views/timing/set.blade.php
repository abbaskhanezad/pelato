...
<script>
    ...
    $('.timing_reserved').each(function(){
        ...
        //alert('aa');
         if (!$(this).hasClass('pelatoreserve')) {
        $(this).addClass("abbas");
    ...
    $(document).on('click', ".abbas", function(){
     swal('این ساعت ها توسط خودتان رزرو شده است');
    });
              ...
              <div class="caption font-dark" style="width: 450px;">
                  ...
                  <span class="caption-subject bold uppercase">زمانبندی اتاقهای مرکز " {{ $center->name }} " برای
                    <span class="week_title">
                      <a href="@if($is_admin) /timing/center/{{ $center->id}}/week/{{ $set_week->id - 1 }}/set @else /timing/week/{{ $set_week->id - 1 }}/set @endif"><i class="fa fa-chevron-right" title="هفته قبل"></i></a>
                      {{ $lingual_set_week }}
                      <a href="@if($is_admin) /timing/center/{{ $center->id }}/week/{{ $set_week->id + 1 }}/set @else /timing/week/{{ $set_week->id + 1 }}/set @endif"><i class="fa fa-chevron-left" title="هفته بعد"></i></a>
                    </span>
                    </span>
              </div>
              <div class="actions">
                <form method="post" id="room_timing_result">
                  ...
                  <a class="btn btn-primary pull-right" href="@if($is_admin) /timing/center/{{ $center->id }}/ @else /timing @endif">بازگشت</a>
