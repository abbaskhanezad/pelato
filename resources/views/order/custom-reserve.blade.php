...
<div class="container">
    <div class="row">
        ...
        <div class="col-sm-6 col-lg-offset-3">
            <form action="{{ route('custom.reserve') }}" method="post" >
                {{ csrf_field() }}
                    <fieldset>
                        ...
                        <div class="row">
                            <div class="form-group col-sm-6 {{ $errors->has('user_name') ? ' has-error' : '' }}">
                                <span class="help-block text-left" > نام و نام خانوادگی رزرو کننده </span>
                            ...
                            <div class="form-group col-sm-6 {{ $errors->has('mobile') ? ' has-error' : '' }}">
                                <span class="help-block text-left" > موبایل رزرو کننده </span>
                                <input type="text" name="mobile" class="form-control" id="mobile" value="{{ old('mobile') }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-sm-6">
                                <span class="help-block text-left">مبلغ رزرو شده</span>
                            ...
                            <div class="form-group col-sm-6 {{ $errors->has('center_type') ? ' has-error' : '' }}">
                                <span class="help-block text-left">نوع پرداخت</span>
                                <select class="form-control" name="status_payment" id="status_payment" style="height: 44px" >
                                    ...
                                    @foreach($status_payments as $status_payment)
                                        @if($status_payment->id!=4 && $status_payment->id!=1)
                                        <option value="{{$status_payment->id}}" @if(old('status_payment') == $status_payment->id ) selected @endif class="form-control">{{ $status_payment->title }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="f1-buttons pull-right">
                            <button type="submit" class="btn btn-success btn-sm"> رزرو</button>
                        </div>
                    </fieldset>
            </form>
        </div>
